...
    {
        public RenderPassEvent passEvent = RenderPassEvent.AfterRenderingTransparents;//默认插到透明完成
...
        RenderTargetHandle passTemplecolorTex;//临时计算图像
        RenderTargetHandle passTemplecolorTex1;//临时计算图像
        string passTag;
...
        {
            ...
            cmd.GetTemporaryRT(passTemplecolorTex1.id, opaquedesc, passfiltermode);
            Blit(cmd, passSource, passTemplecolorTex.Identifier(), passMat);
            //Blit(cmd, passSource, passTemplecolorTex1.Identifier(), passMat);
